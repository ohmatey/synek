---
name: follow
description: "Follow a topic in Synek — set up a living timeline + private serialized story that stays current on a schedule. Use when the user runs /synek:follow, asks to follow/track/monitor a topic over time, wants a recurring digest of a field (a market, a technology, a rivalry, an ongoing story), or wants a timeline that keeps itself up to date AND tells its story chapter by chapter. This is the packaged loop: scope brief → private series → first chapter now → a recurring keeper-chapter routine you can schedule in any MCP client."
