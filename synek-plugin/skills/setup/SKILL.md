...
- **Hosted (power user).** Synek is deployed to a public HTTPS origin (e.g. `https://synek.sector137.io`). The user points the plugin at it by setting **`SYNEK_MCP_URL`** to `https://their-host/api/mcp` before launching Claude Code. The rest is identical, including the 401 → Authorize → token flow; only the origin changes.
...
2. **Diagnose, in order:**
   ...
   `401` means the server is up and is correctly asking you to authorize → go to 2b. A connection-refused / empty response means one of two things. Locally, the server isn't running: start it from the Synek repo with `bun run dev`. Hosted, the origin is wrong or the deploy is down: check the URL and the deploy's health.

   b. **Authorize (the usual fix).** The plugin uses OAuth, so the user grants access in the browser. There is no key to export:
   - First, make sure you're **logged into the same origin the plugin points at**: `http://localhost:3001` locally, or the hosted URL (sign up / log in with email + password). The OAuth consent uses that session.
   ...
   e. **No `synek` MCP server at all?** If the Synek tools are entirely absent (*missing*, not failing), the skills were installed without the plugin (e.g. via `npx skills`), so the MCP connection didn't come along. Add it once, then authenticate as in 2b:
   ```bash
...
- **Local-first by default.** With nothing configured, the plugin talks to your own machine at `http://localhost:3001`. Plain `http://localhost` (not HTTPS) is correct there; it's a loopback to your own box and Claude Code supports OAuth over localhost. The local path is unchanged.
