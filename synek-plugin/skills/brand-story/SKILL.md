...
The user wants a Synek story that **sounds and looks like their Realscript brand**. You are the bridge: read the brand from the **Realscript** MCP, write the story into **Synek** via `write_story`, and dress the canvas with the brand's visual identity. Nothing talks server-to-server — *you* (this client, holding both MCPs) are the only connection between the two apps. That's the point.
...
1. **Confirm BOTH MCPs are live.**
   - Synek: call `list_timelines`. If it errors, run the `setup` flow — the local Synek server is down or unauthenticated.
...
4. **Apply the brand — two layers:**
   - **Voice → the prose.** Write every beat (`write_story` `bodyText`, the `hook`, narration) in the brand's register. Honor the voice schema literally: lead with the high-intensity personality traits, obey the do/don't writing rules, prefer the brand's vocabulary and avoid its banned words, and pitch tone to the spectrum values. The story should be unmistakably *this* brand — a generic-sounding story is a failure even if the facts are right.
   ...
   - **Values/key messages → the throughline.** Let the brand's mission, core values, and key messages shape *what the story is about* and the note it ends on — not just the diction.
...
It must **read** like the brand (specific vocabulary and tone pulled from the kit, not generic AI prose) and, if themed, **look** like the brand (palette applied, contrast clean). The brand kit is the cited source of voice — never fabricate brand attributes. Two apps, one client, one coherent on-brand story: that's the companion-app promise made real.
