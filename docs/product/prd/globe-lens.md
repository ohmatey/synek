...
- **Gated on coverage.** Globe lens entry is hidden (or prominently explains the gap) unless a threshold of nodes carry coordinates.
...
- **Zero coordinates:** the view switcher is **not rendered** (the top bar looks exactly as it does today). The globe remains discoverable via a non-primary affordance ("Add locations to see the globe view") shown in the node detail panel's `location` field area when the timeline has any `location` strings but no `lat`/`lng`, and via ⌘K as a "Set up globe view" entry carrying the backfill prompt.
...
1. **Top-center view switcher** (primary): a segmented control ("Timeline | Globe") floating top-center in the `.top-bar`, between the AppBar identity chips (left) and the canvas toolbar (right). This is the canonical lens toggle — one click switches views, the active segment shows where you are. Visibility follows the coverage gate: hidden entirely at zero coordinated nodes (the top bar looks exactly as it does today); present from ≥1 coordinated node, with the coverage state in the Globe segment's tooltip when below threshold ("5 of 23 nodes located — add more for a fuller picture").
2. **⌘K command palette**: a "Switch to globe view" command in the NAV registry, shown when the current timeline has ≥1 coordinated node. If zero coordinated nodes, a "Set up globe view" entry with the backfill prompt affordance.
...
**R3 — Coverage cold-start.** New timelines will have zero coordinates. The globe entry is hidden/degraded. The friction of adding coordinates to every node is real — it requires a dedicated `apply_patch` call (or inline on creation). If coordinate adoption is slow (metric 1 misses), the lever is improving the backfill prompt UX, not loosening the coverage gate. Do not lower the gate to make the lens look fuller than it is.
...
**Entry/exit wiring:**
- **View switcher (primary):** a `ViewSwitcher` segmented control ("Timeline | Globe") rendered top-center in the `.top-bar` (TimelineCanvas:963-1018). The bar is currently a two-ended flex row (`AppBar` left, `canvas-toolbar` right) — center the switcher with absolute positioning (`left-1/2 -translate-x-1/2`) inside the relatively-positioned bar so it stays truly centered regardless of the chip widths on either side; style it as a `floatChip` group (the established chrome idiom, `chrome.ts`). Radix/shadcn shape: a two-item `ToggleGroup` (`type="single"`, roving focus, `aria-label="Canvas view"`) driving `setLensView`. Render it only when ≥1 coordinated node exists; coverage state goes in the Globe item's tooltip — read `coveragePct` from the in-memory graph (compute client-side; don't round-trip `get_layout_report` for a tooltip). Note the public-viewer case: the switcher shows for any viewer (it's read-only navigation, not an owner mutation), same as the canvas itself.
...
9. Coverage gate: threshold check, low-coverage state with backfill prompt, zero-coverage hidden entry
