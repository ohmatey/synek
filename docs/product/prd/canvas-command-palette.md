...
- A **"Filter results" menu** sits at the **right of the search row** (a compact funnel icon, where the close button used to be) — a multi-select over the result categories present (Actions + each node group, with counts). Untick a category to drop that whole group from the list; a count badge marks an active filter, with "Show all" inside the menu to clear. Palette-local — narrows the result list only, independent of the canvas. The **close button hovers outside** the dialog at the top-right.
...
- **Filter dropdown — `FilterMenu`.** A toolbar `DropdownMenu` of `DropdownMenuCheckboxItem`s over the present kinds (`kindToken`/`kindCounts`), driving the same `hiddenKinds` state the canvas already filters on. `onSelect` is `preventDefault`-ed so the menu stays open across multiple toggles. This **replaced** the kind-filter that used to live buried in the View-Settings popover — one discoverable home. *(Superseded — the standalone `FilterMenu` chip was later **re-merged** into the View-Settings popover as a "Show on timeline" section of `role="checkbox"` rows; `FilterMenu.tsx` was deleted and the settings trigger shows an amber hidden-count badge. One popover now owns the whole canvas view.)*
