...
**Core UX only. Most lovable product before any thought of money.** This repo is the single-user, local-first **Core**. The following are **deliberately out of scope** and must NOT be added until the core feels undeniable (they live in `docs/product/roadmap.md` → Deferred; the gate is defined in `docs/product/product-strategy.md`):
