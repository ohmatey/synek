...
- ~~Multi-tenant hosting~~ **Phase 2 BUILT (2026-06-14, uncommitted):** per-user isolation (every artifact/timeline/story/history read owner-scoped — see `verify:isolation`), open signup + email verification & password reset via Resend (`src/lib/auth/email.ts`), per-user **BYO OpenRouter key** encrypted at rest (`src/lib/crypto/secrets.ts` + `user_settings`), and a single-instance cloud deploy. **Deploy target = the sector137 Kubernetes cluster** (GitOps via loha/ArgoCD; manifests in the `sector137-infra` repo at `kubernetes/overlays/synek/` + `argocd/applications/apps/synek.yaml`; runbook `docs/engineering/deploy-sector137.md`). The public origin is `https://synek.sector137.io`. **Fly.io was retired 2026-06-19 (founder decision)** — `fly.toml` + `deploy-fly.md` removed. Still optional: pointing the plugin at a hosted URL by default
...
| AI | **None in-app by default** — intelligence comes from the user's MCP client. The app exposes an MCP server (`@modelcontextprotocol/sdk`) — HTTP at `/api/mcp` + a stdio binary. **Optional in-app agent** (`src/lib/agent/`, key-gated on `OPENROUTER_API_KEY`): an in-process OpenRouter function-calling loop over the **same** tool registry the MCP server uses (`src/lib/mcp/registry.ts`), reachable from the prompt dialogs' "Run" button via the `runAgent` server fn. **Manual authoring is IN, AI is not** — you can **create** (`createNode`), edit (`editNode`), and place a shared entity (`placeEntityOnTimeline`) nodes in-app via the unified **Add** surface + ⌘K (ADR 0008). That is plain data entry through the same Patch engine — it carries no model — so the *no-in-app-intelligence* inversion is unchanged; only "the canvas is a pure viewer" is superseded |
...
| Auth | **Better Auth** (multi-user since Phase 2 — was single local user), `bearer` plugin. The credential is a long-lived session token (Better Auth 1.6 has no api-key plugin) sent as `Authorization: Bearer <token>`; mint with `bun run issue:key`. Open email/password signup with email verification + password reset via Resend (`src/lib/auth/email.ts`; no-ops without `RESEND_API_KEY`). **Optional Google sign-in** — key-gated on `GOOGLE_CLIENT_ID`+`GOOGLE_CLIENT_SECRET` (both unset = no Google button, local-first default intact); `socialProviders.google` in `src/lib/auth/index.ts`, flag surfaced to the client by `getAuthConfig` (`src/lib/server/auth-config.ts`), button in `AuthForms`. No migration (the `account` table already has the social columns); account linking stays at Better Auth's secure defaults (won't auto-link Google to an **unverified** email/password account — pre-hijack guard). Every data path is **owner-scoped** (per-user isolation); per-user settings (incl. the encrypted BYO OpenRouter key) live in `user_settings` |
| Validation | Zod v4 |
...
React Flow is **client-only** — always render it behind `<ClientOnly>` (SSR touches `window`). Node x-position derives from date via the shared `TimeScale` (`makeTimeScale` in `useTimelineScale.ts`): **cluster-aware sparse-time compression is ON by default** (`DEFAULT_COLLAPSE_GAPS`) — anchors cluster by 3× the median gap (capped at 25y), sparse inter-cluster stretches shrink to 72–144px axis breaks, intra-cluster spacing stays linear; an explicit user toggle (`ScalePref.chosen`) or saved `viewSettings` overrides. y is the packed lane (`layoutLaneY`; `LANE_Y` is the fallback). Nodes are non-draggable (position is owned by the data). `nodeTypes` **and `edgeTypes`** are module-level (memoized). Edges are colored by `kind` with an arrow marker and render via `edges/LabeledEdge.tsx`: React Flow's built-in label pins text to the bezier midpoint — which on a timeline is where a node sits — so the layout solves each label's slot against the node rects (`placeEdgeLabel`, a pure fn in `useTimelineScale.ts`; each placed label becomes an obstacle for the next) and passes absolute coords down. Long labels ellipsize at `LABEL_MAX_W` with the full text on `title`. `apply_patch` warns on connected-but-distant endpoints and `get_layout_report` has a `grouping` section (`src/lib/mcp/graph-shape.ts`). Owner-only **invitation ghosts** (thin lane / axis gap / bare era) can be switched off wholesale (`ScalePref.showSuggestions`) or dismissed one at a time (`synek:ghosts-dismissed:{timelineId}` in localStorage — a UI preference, never graph state); the same "Add to a track" prompt stays reachable from ⌘K so hiding a ghost never loses the action.
